...
```typescript
// 이럴 거면 구태여 타입스크립트를 쓸 이유가 없지요.
('something' as any) * 10;
```

마치 양날검 같군요, 잘 쓰면 빠르게 프로덕트를 개발할 수 있지만 자칫 잘못 쓰면 되려 버그 지옥에 빠지게 되니까요. 
...
이 질문에 답하려면 먼저 서브타입 관계(*subtype relation*)와 탑 타입(*top type*), 그리고 바텀 타입(*bottom type*)을 알아야 합니다.
...
서브타입 관계를 따질 수 있는 타입 시스템에는 탑 타입과 바텀 타입이라 불리는 특별한 타입들이 존재합니다.

**탑 타입**은 **모든 타입의 슈퍼타입**으로 모든 타입의 값을 담을 수 있지만, 모든 타입의 값에 대해 공통적으로 할 수 있는 연산 외에는 그 어떤 연산도 할 수 없다는 점이 특징입니다.
...
**바텀 타입**은 **모든 타입의 서브타입**으로, 모든 타입의 서브타입이어야 하기 때문에 어떤 값도 가지지 못한다는 점이 특징입니다.
타입스크립트에서 바텀 타입은 `never`입니다.

```typescript
const bottom: never = undefined; // Wrong!
const bottom2: never = {}; // Wrong!
const bottom3: never = 1; // Wrong!
```
...
위 정의에 따르면, `any`는 탑 타입임과 동시에 바텀 타입입니다.
모든 타입의 슈퍼타입이자 서브타입이기 때문이죠.
하지만 `any`는 탑 타입의 '탑 타입의 값에는 모든 타입의 값에 적용 가능한 연산 외에 어떤 연산도 적용할 수 없다'라는 특징과, 바텀 타입의 '바텀 타입은 어떤 값도 가질 수 없다'라는 특징은 가지고 있지 않습니다.
이게 과연 가능한 일일까요?

아니요, 전혀 그렇지 않습니다.
탑 타입이라면 모든 타입의 값을 갖기 때문에 안전한 타입이 되려면 당연히 모든 타입의 값에 적용 가능한 연산만 적용할 수 있어야 함이 마땅하며,
바텀 타입이라면 모든 타입의 서브타입이기 때문에 안전한 타입이 되려면 당연히 어떤 값도 가지지 않아야 하기 때문이죠(`boolean` 타입의 서브타입이면서 `number` 타입의 서브타입인 타입을 상상해 보세요!).
즉, **'탑 타입이면서 바텀 타입'인 타입의 존재는 모순 그 자체이며, 그렇기에 `any`는 위험한 타입입니다**.
...
`any`를 위험하게 만드는 성질은 `any`가 탑 타입이면서 바텀 타입이라는 성질이고, `any`의 가치는 모든 자바스크립트 코드를 수용할 수 있다는 점에 있는데 이 특징은 `any`가 탑 타입이라는 성질에서 비롯된 것이니
**탑 타입이기만 한 `any`가 안전한 `any`**라고 할 수 있겠습니다.
...
이러한 제약을 반영해, existential type의 값에 대해 함수를 적용하는 연산은 일반적인 값들과 달리 `$`나 `pipe` 같은 부류의 함수가 아닌 **eliminator**라는 부류의 함수로 추상화되곤 합니다.
