...
**어떤 타입 `M`에 대해 아래의 두 함수, `pure`과 `fish`가 존재할 때, `M`은 모나드입니다.**

```typescript
type Pure = <A>(a: A) => M<A>;
type Fish = <A, B, C>(f: (a: A) => M<B>, g: (a: B) => M<C>) => (a: A) => M<C>;
```
...
function fish<A, B, C>(f: (a: A) => Maybe<B>, g: (a: B) => Maybe<C>): (a: A) => Maybe<C> {
  return (a: A): Maybe<C> => {
    const ma = f(a);
...
중요한 것은 모나드가 바로 이 이론의 개념이라는 점과, 유지니오 모기(*Eugenio Moggi*)라는 컴퓨터 과학자가 범주 의미론에서 **모나드를 이용하여 연산들을 정의하고 추상화 할 수 있다는 걸 발견했다는 점**입니다.
...
마지막으로 각 관점에서 관찰한 것들을 반대 관점에서 얻은 지식으로 설명하여, 두 관점에서 깨달은 내용들을 합치고 모나드의 전체적인 모습을 그려 보도록 하겠습니다.

### 속뜻에서 겉뜻으로, 모나드의 정의는 이렇게 만들어졌다.

모나드의 정의를 다시 떠올려 봅시다.

> **어떤 타입 `M`에 대해 두 함수, `pure`과 `fish`가 존재할 때, `M`은 모나드이다.**

제가 처음 이 정의를 제시했을 때에는 모나드에 대한 충분한 논의가 이루어지지 않았었기 때문에 외적인 요소들만 소개해드리는 데에 그쳤었습니다.
...
타입 `M`의 의미에 대해 알아보았으니 다음으로 `fish` 함수의 의미에 대해 알아보도록 하겠습니다.

`fish` 함수의 시그너쳐는 `<A, B, C>(f: (a: A) => M<B>, g: (a: B) => M<C>) => (a: A) => M<C>`로, 같은 연산을 하는 두 프로그램을 받아 하나로 합치는 형태이며 실제 동작도 마찬가지입니다.
...
즉, `fish` 함수는 합성 가능한 연산들만이 모나드가 될 수 있게 하기 위해 존재하는 제한인 셈입니다.

마지막으로, `pure` 함수의 의미에 대해 알아보도록 하겠습니다.
`pure` 함수의 시그너쳐는 `<A>(a: A) => M<A>`로, 프로그램의 기본적인 형태와 일치합니다. 이 함수가 의미하는 바는 무엇일까요?
이 함수는 일반적인 값을 연산 `M`이 적용된 값인 것 처럼 꾸며 주는 함수입니다. 꾸며 준다니, 무슨 이야기일까요?
...
**모나드란, 자신에 대해 두 함수, `pure`과 `fish`가 존재하여 스스로를 합성할 수 있는 연산을 나타내는 타입 `M`입니다.**
...
- 모나드의 겉뜻: **어떤 타입 `M`에 대해 두 함수, `pure`과 `fish`가 존재할 때, `M`은 모나드이다.**
- 모나드의 속뜻: **어떤 연산이 모나드임이 시사하는 바는 그 연산은 합성할 수 있다는 점이다.**
- 모나드: **자신에 대해 두 함수 `pure`과 `fish`가 존재하여 스스로를 합성할 수 있는 연산을 나타내는 타입 `M`.**
